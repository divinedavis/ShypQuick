-- Bring server-side base prices in line with the SHYP QUICK® Pricing &
-- Driver Strategy update: small (Car) goes from $40 -> $45, large (Truck)
-- from $150 -> $125, so the Car-to-Truck upgrade now bumps the fare by
-- $80 instead of $110.
--
-- Mileage and add-on surcharges are computed client-side and baked into
-- total_cents at request time, so no schema change is needed for those.
...
declare
  ...
  c_car_base     constant int := 4500;
  c_truck_base   constant int := 12500;
begin
